...
resource "aws_route" "local_rt" {
  ...
  route_table_id = coalesce(
    var.local_main_route_table_id,
    data.aws_vpc.local.main_route_table_id,
  )
...
resource "aws_route" "remote_rt" {
  provider = aws.remote
  route_table_id = coalesce(
    var.remote_main_route_table_id,
    data.aws_vpc.remote.main_route_table_id,
  )
...
resource "aws_vpc_peering_connection" "peering" {
  ...
  tags = merge(
    var.tags,
    {
      "Name"          = "Initiator: VPC Peering between default and remote"
